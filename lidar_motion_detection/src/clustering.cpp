...
void Clustering::VoxelClustering(
    ...
    for (int i = 0; i < occupied_ever_free_voxel_indices.size(); ++i){
    	...
    	tsdf_block = tsdf_map_->getTsdfLayerPtr()->getBlockPtrByIndex(voxel_coordinates.first);
	    ...
	    while (stack.size()>0){
		    ...
		    voxblox::Neighborhood<voxblox::Connectivity::kSix>::getFromBlockAndVoxelIndex(voxel_coordinates.first, voxel_coordinates.second, voxels_per_side_, &neighbors);
			...
			voxblox::TsdfVoxel* neighbor_voxel = &tsdf_block->getVoxelByVoxelIndex(coordinates.second);
		       ...
		       if (!neighbor_voxel->clustering_processed && neighbor_voxel->curr_occupied == frame_counter){
...
void Clustering::InducePointClusters(voxblox::AnyIndexHashMapType<int>::type* block2points_map, std::vector<voxblox::HierarchicalIndexIntMap>* blockwise_voxel2point_map, const pcl::PointCloud<pcl::PointXYZ>& all_points, std::vector<std::vector<voxblox::VoxelKey>>* voxel_cluster_ind, std::vector<pcl::PointIndices>* cluster_ind){
	...
	for (auto voxel_cluster : *voxel_cluster_ind){
		...
		for (auto coordinates : voxel_cluster){
			...
			for (auto point_index: ((*blockwise_voxel2point_map)[(*block2points_map)[coordinates.first]])[coordinates.second]){

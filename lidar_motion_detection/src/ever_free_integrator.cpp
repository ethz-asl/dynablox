...
EverFreeIntegrator::EverFreeIntegrator(
  ...
  nh_private_.param<int>("ever_free_occ_initial_buffer", occ_temporal_buffer_,
                         5);
  nh_private_.param<float>("tsdf_voxel_size", voxel_size_,
                         0.2);
}

void EverFreeIntegrator::MakeEverFree(voxblox::BlockIndex &block_index, const int &frame_counter) {  
  ...
  voxblox::TsdfVoxel* tsdf_voxel;
	...
	// Voxel must be unoccupied for the last 5 frames and TSDF-value must be larger than 3/2 voxel_size
	if (tsdf_voxel->last_static > frame_counter - 5){
        ...
        global_voxel_index = voxblox::getGlobalVoxelIndexFromBlockAndVoxelIndex(block_index, voxel_idx, static_cast<int>(voxels_per_side_));
        ...
        for (unsigned int idx = 0u; idx < neighbor_indices.cols(); ++idx) {
          const voxblox::GlobalIndex& neighbor_index =
              neighbor_indices.col(idx);

          voxblox::TsdfVoxel* neighbor_voxel = tsdf_map_->getTsdfLayerPtr()->getVoxelPtrByGlobalIndex(neighbor_index);
...
void EverFreeIntegrator::RemoveEverFree(const voxblox::BlockIndex block_index, const voxblox::VoxelIndex voxel_index, int frame_counter) {
  ...
  voxblox::Neighborhood<voxblox::Connectivity::kEighteen>::getFromBlockAndVoxelIndex(block_index, voxel_index, voxels_per_side_, &neighbors);

  for (auto coordinates: neighbors) {
          tsdf_block = tsdf_map_->getTsdfLayerPtr()->getBlockPtrByIndex(coordinates.first);
          
          if (tsdf_block == nullptr){
          continue;
          }
          
          voxblox::TsdfVoxel* neighbor_voxel = &tsdf_block->getVoxelByVoxelIndex(coordinates.second);
...
void EverFreeIntegrator::updateOccupancyCounter(voxblox::TsdfVoxel* voxel, int frame_counter) {

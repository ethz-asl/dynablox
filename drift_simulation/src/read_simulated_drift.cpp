...
drift_reader::drift_reader(ros::NodeHandle nh,
                                   ros::NodeHandle nh_private) : tf_listener_(tf_buffer_) {                                 
  ...
  pointcloud_sub_ = nh.subscribe("pointcloud", 100,
                                 &drift_reader::poseCallback, this);
...
void drift_reader::poseCallback(const sensor_msgs::PointCloud2 &pointcloud_msg){
	...
	transform.setOrigin(tf::Vector3(pose_data[0], pose_data[1], pose_data[2]));
    	transform.setRotation(tf::Quaternion(pose_data[3], pose_data[4], pose_data[5], pose_data[6]));
            	
        br.sendTransform(tf::StampedTransform(transform, pointcloud_msg.header.stamp, "os1_drift", "map"));  
  	frame_counter += 1;
